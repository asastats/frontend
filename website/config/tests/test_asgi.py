...
class TestAsastatsAsgi:
    """Testing class for :py:mod:`website.config.asgi` module."""
    ...
    def test_config_asgi_sets_application_as_protocoltyperouter(self):
        assert isinstance(self.app, ProtocolTypeRouter)

    def test_config_asgi_sets_websocket_as_authmiddlewarestack(self):
    ...
    def test_config_asgi_sets_websocket_authmiddleware_urlrouter(self):
        assert isinstance(self.router, URLRouter)

    def test_config_asgi_websocket_routes_count(self):
        assert len(self.router.routes) == 1

    def test_config_asgi_defines_historic_user_widget_websocket_url(self):
    ...
    def test_config_asgi_sets_http_as_asgihandler(self):
